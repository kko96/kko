...
public class AdminBoardController {
	...
	public @ResponseBody Map<String, Object> getInfo(@PathVariable String boardCode, @RequestParam Map<String, Object> params) {
		...
		List<FileVO> list = fileService.selectList(params);
		params.put("files", list);
		return params;
	}//getInfo()
	...
	public @ResponseBody String regist(@PathVariable String boardCode, AdminBoardVO vo,
		...
		if(success > 0) {
			if(attachments != null && attachments.length > 0) {	//첨부파일
				...
				if(list.size() > 0) fileService.insertInfo(list);
			}//if end
			if(banner != null && banner.length > 0) {		//이벤트 배너이미지
				List<FileVO> list = fileUtil.saveBanner(banner, boardCode, boardNo);
				if(list.size() > 0) fileService.insertInfo(list);
			}//if end
		}//if end
