...
function putList(list){
	...
	if(list.length > 0){
		$.each(list, function(index, vo){
			var tr = temp.clone();
			var link = $('<a href="<c:url value='/admin/board/event/modify'/>?no=' + vo.boardNo + '">').text(vo.title);
			tr.find("td").eq(0).text(vo.state)
						.next().append(link)
						.next().text(vo.eStartDate + "~" + vo.eEndDate)
...
<div id="wrap" class="hideNav">
	...
	<div id="container">
		<div class="contentWrap">
			<div class="contents">
				<div class="title"><h3>이벤트 목록</h3></div>
				<form action='<c:url value="/admin/board/event/list"/>' name="search" class="search" method="post">
					<fieldset class="box">
				...
				<div class="lOpt">
					...
					<p class="btnBox ar">
						<a href='<c:url value="/admin/board/event/regist"/>' class="rBtn">등록</a>
					</p>
				</div>
			</div><!-- /.contents -->
		</div><!-- /.contentWrap -->
