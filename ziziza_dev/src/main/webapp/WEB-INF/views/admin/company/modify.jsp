...
function putInfo(vo, charge, plants){
	...
	$.each(charge, function(idx, cl){
		var li = $("<li>").text(cl.timeKind + " " + cl.subject  + " " +  cl.price);
		li.appendTo(cList);
	});
	//편의시설
					...
					<form action='<c:url value="/admin/company/modify"/>' name="form" class="form" data-no='<c:out value="${param.no}"/>' method="post" enctype="multipart/form-data">
						<fieldset class="box">
							...
							<div class="addressBox">
								...
								<button type="button" class="openAddr">우편번호 검색</button>
								<span class="message"></span>
							</div>
							<div class="addressBox">
							...
							<div>
								<label for="openTime">영업시작시간</label><input type="time" name="openTime" id="openTime"/>
								<span class="message"></span>
							</div>
							<div>
								<label for="closeTime">영업마감시간</label><input type="time" name="closeTime" id="closeTime"/>
								<span class="message"></span>
							</div>
							<div>
						...
						<fieldset class="fOpt">
							<a href='<c:url value="/admin/company/delete"/>' class="dBtn btn">삭제</a>
							<a href='<c:url value="/admin/company/list"/>' class="lBtn btn">목록</a>
