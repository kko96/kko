				...
				<div class="content">
					<form action='<c:url value="/admin/qna/modify"/>' name="form" class="form" data-no='<c:out value="${param.no}"/>' method="post" enctype="multipart/form-data">
						<fieldset class="box">
							...
							<div>
								<label for="qstTitle">제목</label><input type="text" id="qstTitle" name="qstTitle" disabled/>
							</div>
							<div>
								<label for="qstContents">문의 내용</label><textarea id="qstContents" name="qstContents" rows="8"  disabled></textarea>
							</div>
							<div>
								<label for="answerName">답변자</label><input type="text" name="answerName" id="answerName" disabled/>
						...
						<fieldset class="fOpt">
							<a href='<c:url value="/admin/qna/delete"/>' class="dBtn btn">삭제</a>
							<a href='<c:url value="/admin/qna/list"/>' class="lBtn btn">목록</a>
